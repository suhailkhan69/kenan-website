...
const AppointmentList = () => {
  ...
  const updateStatus = async (id, status) => {
    try {
      const response = await axios.patch(`https://kenan-backend.onrender.com/api/appointments/${id}/status`, { status });
  ...
  const updateCheckIn = async (id, checkedIn) => {
    try {
      const response = await axios.patch(`https://kenan-backend.onrender.com/api/appointments/${id}/checkedin`, { checkedIn });
      const updatedAppointment = response.data.data;
      setAppointments(prevAppointments => prevAppointments.map(appointment =>
        appointment._id === updatedAppointment._id ? updatedAppointment : appointment
      ));
  ...
  const handleAddFormSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`https://kenan-backend.onrender.com/api/appointments`, newAppointment);
